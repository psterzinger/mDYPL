...
function eq_bin!(res, param, kappa, gamma, lambda, s; verbose = false,  int_abstol = 1e-10, int_reltol = 1e-10) 
    ...
    q_1 = cdf(normal, -t_1) 
    q_2 = cdf(normal, -t_2) 
    sigma_tau_frac = 1 / (2 * sqrt(kappa) * sigma * tau)
    ...
    res[6] = b_kappa_frac * sigma_tau_frac * lambda^2 + b * rho^2 * sigma_tau_frac + gts * q_1 - lambda_term - 2 * ((gamma * mu)^2 + kappa * sigma^2) * sigma_tau_frac^2 
    if verbose
...
function eq_bin_integrands!(g, x, kappa, gamma, mu, sigma, b, theta, tau, rho; cutoff = 50000) 
    ...
    ## mu 
    g[1] = 2.0 * tdens * ∂²ζ(Z) * prox_val + theta * b
    ## b 
    g[2] = pre_mult / (1.0 + b * ∂²ζ(prox_val)) - 1.0 + b / (sqrt(kappa) * sigma * tau)
    ##sigma 
    g[3] = pre_mult * ∂ζ(prox_val)^2 - rho^2 / b 
    g[abs.(g).>cutoff] .= NaN 
end 
